  ...
  function fnFeedAdd() {
    ...
    fetch("http://localhost:3010/instafeed/", {
      ...
      .then(data => {
        ...
        alert("insta_tbl_feed로 데이터 삽입되었습니다.");
    ...
    <Container maxWidth="sm">
      ...
      >
        ...
        <Box display="flex" alignItems="center" margin="normal" fullWidth>
          <input
            // accept="image/*"
            accept="image/*, video/mp4, video/quicktime" // 이미지 및 일반적인 동영상 포맷 허용
            style={{ display: 'none' }}
            id="file-upload"
            type="file"
          ...
          {files.length > 0 && (
            <Box sx={{ display: "flex", gap: 1, marginLeft: 2 }}>
              {files.map((file, idx) => {
                // 파일 타입이 'video/'로 시작하면 Avatar 대신 아이콘 표시 또는 다른 처리
                if (file.type.startsWith('video/')) {
                  return (
                    <Avatar key={idx} sx={{ width: 56, height: 56, bgcolor: 'grey' }}>
